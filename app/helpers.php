...
function g_isTronToken($currency) {
...
function g_convertCoinToSymbol($coin) {
    ...
    else if (stripos($coin, 'USDT') !== false) {
        return substr($coin, 0, 4);
    }

    return $coin;
}

function g_getMasterCurrency($currency) {
    if (g_isERC20Token($currency)) {
        return 'ETH';
    }
    if (g_isBinanceToken($currency)) {
        return 'BNB';
    }
    if (g_isTronToken($currency) || $currency == 'USDT') {
        return 'TRX';
    }
...
function my_encrypt($data, $key)
{
    ...
    return base64_encode_url(base64_encode($val));
}

function my_decrypt($data, $key)
{
    $data = base64_decode(base64_decode_url($data));
